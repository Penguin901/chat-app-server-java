...
public enum ErrorCode {
    ...
    INVALID_TOKEN(HttpStatus.UNAUTHORIZED, "유효하지 않은 요청입니다."), // 잘못된 토큰인 경웅
    ...
    UNAUTHORIZED_ACCESS(HttpStatus.UNAUTHORIZED, "로그인에 실패했습니다.."),

    // 인가 실패 (Authorization) - 403
    FORBIDDEN(HttpStatus.FORBIDDEN, "권한이 없는 사용자입니다."),

    // UserException
    USER_PROVIDER_MISMATCH(HttpStatus.CONFLICT, "다른 간편로그인 계정으로 로그인하 해 주세요. "),
    USER_NOT_FOUND(HttpStatus.NOT_FOUND, "로그인이 필요합니다."), // 404
    USERNAME_ALREADY_EXISTS(HttpStatus.CONFLICT, "이미 사용중인 아이디 입니다."),
    ...
    INVALID_PARTICIPANTS_COUNT(HttpStatus.BAD_REQUEST, "유효하지 않은 요청입니다."),
